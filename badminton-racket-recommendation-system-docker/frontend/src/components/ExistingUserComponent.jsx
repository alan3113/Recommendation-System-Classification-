...
function ExistingUserComponent() {
    ...
    function handleAddRacket(newRacket) {
        const len = racketList.filter(element => element.racketName === newRacket).length
        if(newRacket!='' && len === 0) {
            const racketObject = {
                racketName : newRacket,
                rating : 0
            }
    ...
    return(
        <div className="flex flex-col w-full items-center gap-3">
            ...
            <GetRecommendationComponent hidden={racketList.length == 0}/>
        </div>
    )
}
